  ...
  for (var i = 0; i<tables.length; i++) {
    ...
    for (var r = 0; r<rows.length; r++) {
      ...
      for (var c = 0; c<headers.length; c++) {
        if (c < cells.length) {
          if (cells[c].dataset.label) {
            // for yaml tables generated by render-table.html, pick the header by data-label, not index (e.g. in case of rowspans)
            cells[c].dataset.label = Array.from(headers).find(function (th) {
                                        return th.dataset.label === cells[c].dataset.label;
                                     }).innerText;
          } else {
